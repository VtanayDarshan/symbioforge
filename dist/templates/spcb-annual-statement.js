...
exports.SPCBAnnualStatementTemplate = void 0;
class SPCBAnnualStatementTemplate {
    /**
     ...
     */
    static generateReport(factory) {
        const date = new Date().toLocaleDateString('en-IN');
        let wasteList = factory.declaredWasteOutputs.map(w => `  - [${w.category}] ${w.name}: ${w.estimatedDailyVolumeKg} kg/day (${w.physicalForm})`).join('\n');
        if (!wasteList) {
...
exports.SPCBAnnualStatementTemplate = SPCBAnnualStatementTemplate;
